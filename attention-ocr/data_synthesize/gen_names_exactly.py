...
def get_image(bg, fontpath, text):
    ...
    from_valid_coor_h = get_valid_coord(bot_left_y-text_size[1] - 20 + np.random.randint(15), img.shape[0])
    to_valid_coor_h = get_valid_coord(bot_left_y - text_size[1]+fnt.getsize(text)[1] + 20 - np.random.randint(15), img.shape[0])
    from_valid_coor_w = get_valid_coord(bot_left_x - 20 + np.random.randint(15), img.shape[1])
    to_valid_coor_w = get_valid_coord(bot_left_x+fnt.getsize(text)[0] + 20 - np.random.randint(15), img.shape[1])
...
if __name__ == '__main__':
    ...
    names, freqs = read_names('./data/name_to_gen.txt')
    ...
    print("Total gen", sum(freqs))
    for name, freq in progressbar(zip(names, freqs)):
        for _ in range(freq):
